...
# tickers_nifty50 = "MINDTREE.NS AFFLE.NS NAUKRI.NS"
...
df = pd.read_csv('Indices_tickerlists.csv')
indices = df['Name'].to_list()

# Define the initial ticker list for memory
# df_ticker_symbol = df['Ticker_Symbols'] 
# tickers = df_ticker_symbol.iloc[0]
...
# -- Get list of events
# Nifty Index input
option = st.sidebar.selectbox('Select various NIFTY Index stocks?',indices)
st.sidebar.write('You selected:', option)

START = st.sidebar.date_input('Start Date', datetime.date(2010,1,1))

# Portfolio amount
portfolio_val = st.sidebar.number_input("Enter your Investment amount :", value=10000)
# print(portfolio_val)

# Fetches the tickers symbol list for given Index from Indices_tickerlist.csv file
df_option = df[df['Name']==option]
tickers = df_option['Ticker_Symbols'].values
tickers = str(tickers).replace('[','').replace(']','')
...
# df_allocation.rename_axis(index = "Stocks", columns = "Quantity")
...
df_allocation.columns = ["Quantity"]
